...
def run_backtest_year(df, year, initial_capital=100000, window=30, entry_z=-2.0, exit_z=0.5):
    ...
    if df_year.empty:
        return None, f"No data for {year}"
    
    start_date = df_year.index[0]
    
    # Reset simulation to start in the target year with 100k
...
def generate_report(year):
    ...
    results, err = run_backtest_year(df, year)
    ...
    ax1.plot(results.index, results['TotalValue'], color='#1f77b4', linewidth=2.5, label='Portfolio Value')
    ax1.fill_between(results.index, 100000, results['TotalValue'], color='#1f77b4', alpha=0.1)
    ax1.axhline(100000, color='black', linestyle='--', alpha=0.5)
    ax1.set_title(f'{year} Portfolio Performance (Starting 100k TL)', fontsize=14, fontweight='bold')
    ...
    ax3.set_title(f'{year} Time Allocation')
    ...
    plt.tight_layout()
    file_name = f'roic_{year}_report.png'
    plt.savefig(file_name, dpi=300)
    print(f"Report saved as {file_name}. ROIC: {roic:.2f}%")

if __name__ == "__main__":
    generate_report(2026)
